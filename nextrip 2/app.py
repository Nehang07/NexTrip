...
DB_PATH = os.path.join(BASE_DIR, "nextrip.db")
...
def init_db():
    ...
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS flights (
            ...
            airline TEXT NOT NULL, flight_number TEXT NOT NULL,
            origin TEXT NOT NULL, destination TEXT NOT NULL,
            depart_time TEXT NOT NULL, arrive_time TEXT NOT NULL,
            duration_mins INTEGER NOT NULL, price INTEGER NOT NULL,
            seats_available INTEGER NOT NULL, cabin_class TEXT NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS hotels (
            ...
            star_rating INTEGER NOT NULL, price_per_night INTEGER NOT NULL,
            rating REAL NOT NULL, reviews_count INTEGER NOT NULL,
            amenities TEXT NOT NULL, img TEXT NOT NULL, address TEXT NOT NULL
        );

        CREATE TABLE IF NOT EXISTS buses (
            id INTEGER PRIMARY KEY,
            operator TEXT NOT NULL, bus_type TEXT NOT NULL,
            origin TEXT NOT NULL, destination TEXT NOT NULL,
            depart_time TEXT NOT NULL, arrive_time TEXT NOT NULL,
            duration_mins INTEGER NOT NULL, price INTEGER NOT NULL,
        ...
        CREATE TABLE IF NOT EXISTS bookings (
            ...
            user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
            booking_type TEXT NOT NULL,           -- 'flight' | 'hotel' | 'bus'
            item_id INTEGER NOT NULL,             -- flight/hotel/bus id
            details_json TEXT NOT NULL,           -- passengers, dates, seats, etc.
    ...
    if db.execute("SELECT COUNT(*) AS c FROM flights").fetchone()["c"] == 0:
        from travel_data import generate_flights, generate_hotels, generate_buses
        for f in generate_flights():
            db.execute(
                """INSERT INTO flights (id, airline, flight_number, origin, destination, depart_time,
                   arrive_time, duration_mins, price, seats_available, cabin_class, is_international)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
                (f["id"], f["airline"], f["flight_number"], f["origin"], f["destination"], f["depart_time"],
                 f["arrive_time"], f["duration_mins"], f["price"], f["seats_available"], f["cabin_class"],
                 int(f["is_international"])),
            )
        for h in generate_hotels():
            db.execute(
                """INSERT INTO hotels (id, name, city, country, star_rating, price_per_night,
                   rating, reviews_count, amenities, img, address) VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
                (h["id"], h["name"], h["city"], h["country"], h["star_rating"], h["price_per_night"],
                 h["rating"], h["reviews_count"], json.dumps(h["amenities"]), h["img"], h["address"]),
            )
        for b in generate_buses():
            db.execute(
                """INSERT INTO buses (id, operator, bus_type, origin, destination, depart_time,
                   arrive_time, duration_mins, price, seats_available) VALUES (?,?,?,?,?,?,?,?,?,?)""",
                (b["id"], b["operator"], b["bus_type"], b["origin"], b["destination"], b["depart_time"],
                 b["arrive_time"], b["duration_mins"], b["price"], b["seats_available"]),
            )
        db.commit()
    db.close()
...
# ---------------------------------------------------------------------------
# Cities (shared reference data for flights/hotels/buses search forms)
# ---------------------------------------------------------------------------

@app.route("/api/cities")
def list_cities():
    from travel_data import CITIES
...
def search_flights():
    ...
    if sort == "duration":
        ...
        results.sort(key=lambda r: r["depart_time"])
    else:
...
def search_hotels():
    ...
    if sort == "price-low":
        ...
        results.sort(key=lambda r: r["price_per_night"], reverse=True)
    else:
...
#
# This behaves like a real test-mode payment gateway (Luhn-validated card
# numbers, recognizable test-card outcomes, a simulated processing delay)
# without making any external network calls or moving real money. Swap the
# body of `run_mock_gateway()` for a real Stripe/Razorpay server-side call
# when you're ready to go live — the request/response contract here is
# designed to match what a real gateway integration would look like.
# ---------------------------------------------------------------------------
...
def card_brand(card_number):
    ...
    if digits.startswith("4"):
        return "Visa"
    if digits[:2] in ("51", "52", "53", "54", "55") or digits[:4] in ("2221",):
        return "Mastercard"
...
# Recognized test cards (same convention widely used by real gateways in test mode)
DECLINE_CARDS = {"4000000000000002", "4000000000009995", "4000000000000069"}


def run_mock_gateway(card_number, expiry, cvv, amount):
    digits = "".join(c for c in card_number if c.isdigit())

    if not digits or not luhn_valid(digits):
        return {"status": "declined", "reason": "Invalid card number"}
    if not cvv or not cvv.isdigit() or len(cvv) not in (3, 4):
        return {"status": "declined", "reason": "Invalid CVV"}
    if not expiry or len(expiry) != 5 or expiry[2] != "/":
        return {"status": "declined", "reason": "Invalid expiry date"}
    try:
        exp_month, exp_year = int(expiry[:2]), int("20" + expiry[3:])
        if not (1 <= exp_month <= 12):
            return {"status": "declined", "reason": "Invalid expiry month"}
        now = datetime.now(timezone.utc)
        if (exp_year, exp_month) < (now.year, now.month):
            return {"status": "declined", "reason": "Card has expired"}
    except ValueError:
        return {"status": "declined", "reason": "Invalid expiry date"}

    if digits in DECLINE_CARDS:
        return {"status": "declined", "reason": "Your card was declined by the issuing bank"}

    return {
        "status": "succeeded",
        "gateway_ref": "TXN" + secrets.token_hex(6).upper(),
        "card_brand": card_brand(digits),
        "card_last4": digits[-4:],
    }
...
def checkout_pay():
    data = request.get_json(force=True) or {}
    booking_type = data.get("booking_type")  # 'flight' | 'hotel' | 'bus'
    item_id = data.get("item_id")
    details = data.get("details") or {}
    amount = data.get("amount")
    card = data.get("card") or {}

    if booking_type not in ("flight", "hotel", "bus") or not item_id or not amount:
        return jsonify({"error": "Missing booking information"}), 400

    db = get_db()
    table = {"flight": "flights", "hotel": "hotels", "bus": "buses"}[booking_type]
    item = db.execute(f"SELECT * FROM {table} WHERE id=?", (item_id,)).fetchone()
    if not item:
        return jsonify({"error": f"{booking_type.title()} not found"}), 404

    result = run_mock_gateway(card.get("number", ""), card.get("expiry", ""), card.get("cvv", ""), amount)

    if result["status"] != "succeeded":
        return jsonify({"status": "declined", "reason": result["reason"]}), 402

    booking_ref = "NXT" + secrets.token_hex(4).upper()
    now = datetime.now(timezone.utc).isoformat()
    uid = session["user_id"]

    cur = db.execute(
        """INSERT INTO bookings (booking_ref, user_id, booking_type, item_id, details_json, total_amount, status, created_at)
           VALUES (?,?,?,?,?,?,?,?)""",
        (booking_ref, uid, booking_type, item_id, json.dumps(details), amount, "confirmed", now),
    )
    booking_id = cur.lastrowid
    db.execute(
        """INSERT INTO payments (booking_id, user_id, amount, card_brand, card_last4, gateway_ref, status, created_at)
           VALUES (?,?,?,?,?,?,?,?)""",
        (booking_id, uid, amount, result["card_brand"], result["card_last4"], result["gateway_ref"], "succeeded", now),
    )
    db.commit()
...
def list_bookings():
    db = get_db()
    rows = db.execute(
        "SELECT * FROM bookings WHERE user_id=? ORDER BY created_at DESC", (session["user_id"],)
    ).fetchall()

    out = []
    for r in rows:
        table = {"flight": "flights", "hotel": "hotels", "bus": "buses"}[r["booking_type"]]
        item = db.execute(f"SELECT * FROM {table} WHERE id=?", (r["item_id"],)).fetchone()
        item_dict = dict(item) if item else {}
        if r["booking_type"] == "hotel" and item_dict.get("amenities"):
            item_dict["amenities"] = json.loads(item_dict["amenities"])
        out.append({
            "id": r["id"], "booking_ref": r["booking_ref"], "booking_type": r["booking_type"],
            "details": json.loads(r["details_json"]), "total_amount": r["total_amount"],
            "status": r["status"], "created_at": r["created_at"], "item": item_dict,
        })
    return jsonify(out)
